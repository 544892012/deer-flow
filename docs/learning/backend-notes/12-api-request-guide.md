...
| 文件 | 路由前缀 | 设计理念 | 适用场景 |
|------|---------|---------|---------|
| **`backend/app/gateway/routers/runs.py`** | `/api/runs` | **无状态运行** (Stateless) | 一次性交互，无需预先创建线程 |
| **`backend/app/gateway/routers/thread_runs.py`** | `/api/threads/{thread_id}/runs` | **线程化运行** (Stateful) | 有状态的连续对话，需要历史上下文 |
